<template>
  <div class="flex flex-wrap justify-center gap-8 p-4">
    <div 
      ...
      <div :id="`chart-container-${index}`" class="w-full h-48 flex items-center justify-center"></div>
      
      <p class="mt-4 text-lg text-white font-bold">{{ teamData.team }}</p>
      
      <div class="text-sm text-white mt-2 text-center">
        <span>Sukcesy: <span class="text-green-400 font-bold">{{ teamData.success }}</span></span>
        <span class="mx-2">|</span>
        <span>Porażki: <span class="text-red-400 font-bold">{{ teamData.failure }}</span></span>
      </div>
    </div>
  </div>
</template>
