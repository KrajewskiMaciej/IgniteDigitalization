...
function splitLabelIntoLines(text: string, maxLength = 15): string[] {
  ...
  if (words.length === 1) { return [text.substring(0, Math.ceil(text.length / 2)), text.substring(Math.ceil(text.length / 2))]; }
  let bestSplitIndex = 0; let bestDiff = text.length;
  for (let i = 1; i < words.length; i++) {
    const l1 = words.slice(0, i).join(' '); const l2 = words.slice(i).join(' ');
    const diff = Math.abs(l1.length - l2.length);
    if (diff < bestDiff) { bestDiff = diff; bestSplitIndex = i; }
  }
...
const drawBoard = () => {
  ...
  labelsX.value.forEach((label: string, i: number) => { svg.append("text").attr("x", i * cellSize.value + marginLeft.value + cellSize.value / 2).attr("y", boardSizeY.value + marginTop.value + 20).attr("text-anchor", "middle").attr("font-size", cellSize.value * 0.25).attr("fill", "white").text(label); });
  if (props.config.labelsUp) { props.config.labelsUp.forEach((label, i) => { const centerX = marginLeft.value + (2 * i + 1) * cellSize.value; const baseY = marginTop.value - 20; const lines = splitLabelIntoLines(label); const textElement = svg.append("text").attr("x", centerX).attr("y", baseY - ((lines.length - 1) * 15)).attr("text-anchor", "middle").attr("font-size", cellSize.value * 0.25).attr("fill", "white"); lines.forEach((line, j) => { textElement.append("tspan").attr("x", centerX).attr("dy", j > 0 ? 15 : 0).text(line); }); }); }
  labelsY.value.forEach((label: string, i: number) => { svg.append("text").attr("x", marginLeft.value - 20).attr("y", (props.config.rows - 1 - i) * cellSize.value + marginTop.value + cellSize.value / 2).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("font-size", cellSize.value * 0.25).attr("fill", "white").text(label); });
  if (props.config.labelsRight) { props.config.labelsRight.forEach((label, i) => { svg.append("text").attr("transform", `translate(${marginLeft.value + boardSizeX.value + 20}, ${marginTop.value + (props.config.rows - i * 2 - 1) * cellSize.value}) rotate(-90)`).attr("text-anchor", "middle").attr("dominant-baseline", "middle").attr("font-size", cellSize.value * 0.25).attr("fill", "white").text(label); }); }
  ...
  svg.append("text").attr("transform", `translate(${marginLeft.value - 30}, ${marginTop.value + boardSizeY.value / 2}) rotate(-90)`).attr("text-anchor", "middle").attr("font-size", cellSize.value * 0.3).attr("font-weight", "bold").attr("fill", "white").text(descLeft);
  ...
  for (let i = 0; i < props.config.cols; i += 2) { svg.append("line").attr("x1", marginLeft.value + i * cellSize.value).attr("y1", marginTop.value + boardSizeY.value).attr("x2", marginLeft.value + (i + 2) * cellSize.value).attr("y2", marginTop.value + boardSizeY.value).attr("stroke", borderColors[(i / 2) % borderColors.length]).attr("stroke-width", 3); }
  for (let i = 0; i < props.config.rows; i += 2) { svg.append("line").attr("x1", marginLeft.value).attr("y1", marginTop.value + boardSizeY.value - i * cellSize.value).attr("x2", marginLeft.value).attr("y2", marginTop.value + boardSizeY.value - (i + 2) * cellSize.value).attr("stroke", borderColors[(i / 2) % borderColors.length]).attr("stroke-width", 3); }
  for (let i = 0; i < props.config.rows; i += 2) { svg.append("line").attr("x1", marginLeft.value + boardSizeX.value).attr("y1", marginTop.value + boardSizeY.value - i * cellSize.value).attr("x2", marginLeft.value + boardSizeX.value).attr("y2", marginTop.value + boardSizeY.value - (i + 2) * cellSize.value).attr("stroke", borderColors[(i / 2) % borderColors.length]).attr("stroke-width", 3); }
    ...
    Object.entries(grouped).forEach(([key, group]) => {
      ...
      group.forEach((pawn: Pawn, index: number) => {
        ...
        pawnGroup.append("path")
          ...
          .attr("transform", `translate(${baseX + offsetX}, ${baseY + offsetY}) scale(${baseScale}) translate(${-originalCenterX}, ${-originalCenterY})`);
