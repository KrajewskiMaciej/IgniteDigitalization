...
const processedPawns = computed(() => {
  ...
  // Mapowanie nazw na indeksy ćwiartek:
  ...
  return props.pawns.map((pawn, index) => {
    ...
    switch (quadrant) {
      ...
      case 2: targetX = -pctX * halfCols; targetY = -pctY * halfRows; break // Bottom-Left
      case 3: targetX =  pctX * halfCols; targetY = -pctY * halfRows; break // Bottom-Right
...
const drawBoard = (animate = true) => {
  ...
  const quadConfigs = [
    { x: -halfCols, y: 0, label: names[0], color: colors[0] }, // TL
    { x: 0, y: 0, label: names[1], color: colors[1] },        // TR
    { x: -halfCols, y: -halfRows, label: names[2], color: colors[2] }, // BL
    { x: 0, y: -halfRows, label: names[3], color: colors[3] },        // BR
  ]
  ...
  quadConfigs.forEach(q => {
    ...
    svg.append('text')
      .attr('x', labelPos.screenX).attr('y', labelPos.screenY)
