<template>
  <div>
    ...
    <div class="flex flex-col sm:flex-row w-full items-stretch justify-center gap-5 mt-3 md:mt-5">
      ...
      <div class="border-2 border-lgray-accent py-2 px-2 rounded-md flex-1 text-center flex flex-col justify-between">
...
<script setup lang="ts">
  ...
  const removeColor = (index: number) => { // Jawne typowanie: number
  ...
  const updateBorderColor = (index: number, value: string) => { // Jawne typowanie: number, string
    const updatedColors = [...props.borderColors];
