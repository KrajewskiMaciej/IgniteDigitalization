...
function createParticles() {
    ...
    const createParticleCloud = (count: number, color: string, size: number, spread: {x:number, y: number, z:number}, zOffset: number) => {
...
function createDice(color: string) {
  ...
  const faces: { positions: [number, number, number][], rotation: [number, number, number] }[] = [
    ...
    { positions: [[-s, s, -d], [s, -s, -d]], rotation: [0, Math.PI, 0] },
    { positions: [[-d, 0, 0], [-d, s, s], [-d, -s, -s]], rotation: [0, -Math.PI / 2, 0] },
    { positions: [[d, s, s], [d, s, -s], [d, -s, s], [d, -s, -s]], rotation: [0, Math.PI / 2, 0] },
    { positions: [[-s, d, -s], [-s, d, s], [s, d, -s], [s, d, s], [0, d, 0]], rotation: [-Math.PI / 2, 0, 0] },
    { positions: [[-s, -d, s], [s, -d, s], [-s, -d, -s], [s, -d, -s], [0, -d, s], [0, -d, -s]], rotation: [Math.PI / 2, 0, 0] },
  ]
...
const createOrbitingObjects = () => {
  ...
  const objects = [
    ...
    { type: 'pawn', color: COLORS.pawnGreen, orbitRadius: 4.5, orbitSpeed: -0.12, orbitTilt: -0.6, startAngle: Math.PI * 0.4 },
    { type: 'pawn', color: COLORS.pawnNavy, orbitRadius: 3.4, orbitSpeed: 0.22, orbitTilt: 0.8, startAngle: Math.PI * 1.1 },
    { type: 'pawn', color: COLORS.pawnYellow, orbitRadius: 5.0, orbitSpeed: -0.08, orbitTilt: -0.3, startAngle: Math.PI * 1.7 },
    { type: 'dice', color: COLORS.secondary, orbitRadius: 3.0, orbitSpeed: -0.25, orbitTilt: -0.9, startAngle: Math.PI * 0.2 },
    { type: 'dice', color: COLORS.secondary, orbitRadius: 4.8, orbitSpeed: 0.1, orbitTilt: 0.5, startAngle: Math.PI * 1.4 },
  ]
