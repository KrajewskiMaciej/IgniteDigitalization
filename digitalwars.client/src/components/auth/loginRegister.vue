<template>
  <div v-if="props.isVisible" class="fixed inset-0 flex items-center justify-center z-50">
    ...
    <div
      class="bg-primary text-white rounded-lg relative z-10 border-2 border-accent transition-all duration-300 p-6 sm:p-8 md:p-10 lg:p-12 max-h-[90vh] overflow-y-auto w-full max-w-lg"
      :class="
        props.isVisible ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-4 opacity-0'
      "
    >
      <button
        @click="closeModal"
      ...
      <div
        class="flex mb-1 sm:mb-2 md:mb-3 lg:mb-4 gap-3"
        ...
        <button
          ...
          :class="
            activeView === 'login'
              ...
              : 'bg-tertiary transition delay-150 duration-300 ease-in-out hover:-translate-y-2 hover:scale-105 hover:bg-accent/70'
          "
      ...
      <div
        class="w-100 h-0.5 mb-1 sm:mb-2 md:mb-3 lg:mb-4 bg-accent"
        v-if="activeView === 'login' || activeView === 'register'"
      ></div>
