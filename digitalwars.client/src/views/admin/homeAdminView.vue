<template>
    <div class="w-full text-white">
        <div class="m-4 px-2 py-2 border-2 border-lgray-accent rounded-md bg-tertiary">
            ...
            <div v-else-if="activeGames.length === 0" class="text-center py-4 text-gray-400">Brak aktywnych gier. Utwórz nową, aby zacząć.</div>
...
<script setup lang="ts">
  ...
  const fetchActiveGames = async () => {
    ...
    } catch (error: any) { // FIX: Jawne typowanie błędu
      const errorMessage = error.response?.data?.message || error.message || "Wystąpił nieoczekiwany błąd.";
