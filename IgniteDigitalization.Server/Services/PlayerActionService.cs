...
namespace backend.Services
{
    ...
    public class PlayerActionService : IPlayerActionService
    {
        ...
        public async Task<object> PlayCardAsync(int cardId, int? enablerId, CardDataDto cardData, bool wasSuccess)
        {
            ...
            // Dla kart Fazy 2 ("Rynkowa") wagi są mnożone przez mnożnik przygotowania.
            ...
            bool isPhase2Card = cardEntity.Phase?.Phase_Name == "Rynkowa";
        ...
        {
            var rowsUpdated = await _context.GameLogs
                .Where(gl => gl.Games_Logs_Id == logId && gl.Is_Approved == false)
                .ExecuteUpdateAsync(gl => gl.SetProperty(g => g.Is_Approved, true));

            if (rowsUpdated == 0)            {
                _logger.LogWarning("[ApproveLogAsync] Nie można zatwierdzić logu o ID {LogId}. Log nie istnieje lub jest już zatwierdzony.", logId);
                throw new Exception("Nie można zatwierdzić tej sugestii. Log nie istnieje lub jest już zatwierdzony.");
            }
        ...
        private async Task ExecuteCardEffects(GameLog gameLogEntry)
        {
            ...
            var logEntryWithSpecs = await _context.GameLogs
                ...
                .Include(gl => gl.Cards).ThenInclude(c => c!.Phase)
            ...
            if(logEntryWithSpecs.Cards?.Phase?.Phase_Name == "Wejście na rynek")
            {
                ...
                if(rynkowaPhase != null)
                {
                    ...
                    await _economyService.ApplyPreparationMultiplierAsync(logEntryWithSpecs.Games_Id, logEntryWithSpecs.Teams_Id);
                    _logger.LogInformation("[ExecuteCardEffects] Drużyna {TeamId} weszła w Fazę Rynkową. Zaktualizowano Current_Phase_Id i zastosowano mnożnik przygotowania.", team.Teams_Id);
