    ...
    public class PlayerQueryService : IPlayerQueryService
    {
        ...
        .FirstOrDefaultAsync(t => t.Teams_Token == teamToken);
            ...
            return new
            {
                ...
                currentStage = team.CurrentPhase?.Phase_Name == "Rynkowa" ? 2 : 1,
                boardConfig = new
                {
        ...
        public async Task<IEnumerable<object>> GetProcessPawnsForBoardAsync(int gameId, int teamId, int boardId)
        {
            ...
            var allWeights = await _context.CardWeights
                ...
                .Select(cw => new
                {
                    ...
                    cw.Weights_X,
                    cw.Weights_Y,
                    PhaseName = cw.Cards.Phase != null ? cw.Cards.Phase.Phase_Name : null
                })
                .ToListAsync();
            ...
            // Mapa 1 = karty non-Rynkowa (null, "Przygotowawcza", "Wejście na rynek")
            // Mapa 2 = karty "Rynkowa"
            // MaxPos = Σ(wagi Mapy1) + Σ(wagi Mapy2) × PrepMultiplier_Max
            var result = pawns.Select(pawn =>
            {
                ...
                double maxPosX = weights.Where(cw => cw.PhaseName != "Rynkowa").Sum(cw => cw.Weights_X)
                               + weights.Where(cw => cw.PhaseName == "Rynkowa").Sum(cw => cw.Weights_X) * multiplier;

                double maxPosY = weights.Where(cw => cw.PhaseName != "Rynkowa").Sum(cw => cw.Weights_Y)
                               + weights.Where(cw => cw.PhaseName == "Rynkowa").Sum(cw => cw.Weights_Y) * multiplier;
        ...
        public async Task<IEnumerable<object>> GetRivalPawnsForBoardAsync(int gameId, int boardId)
        {
            ...
            // Formuła per proces (taka sama jak gameBoardCartesian): Σ(wagi Mapy1) + Σ(wagi Mapy2) × PrepMultiplier_Max
            ...
            foreach (var teamId in teamIds)
            {
                ...
                var processMaxes = teamProcesses.Select(proc =>
                {
                    ...
                    double maxX = weights.Where(cw => cw.PhaseName != "Rynkowa").Sum(cw => cw.Weights_X)
                                + weights.Where(cw => cw.PhaseName == "Rynkowa").Sum(cw => cw.Weights_X) * multiplier;
                    double maxY = weights.Where(cw => cw.PhaseName != "Rynkowa").Sum(cw => cw.Weights_Y)
                                + weights.Where(cw => cw.PhaseName == "Rynkowa").Sum(cw => cw.Weights_Y) * multiplier;
